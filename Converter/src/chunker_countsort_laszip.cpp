	...
	vector<function<void(int64_t)>> createAttributeHandlers(laszip_header* header, uint8_t* data, laszip_point* point, Attributes& inputAttributes, Attributes& outputAttributes) {
		...
		{ // STANDARD LAS ATTRIBUTES
			...
			auto classification = [data, point, header, offsetClassification, attributeClassification](int64_t offset) {
				data[offset + offsetClassification] = (point->extended_classification > 31 ? point->extended_classification : point->classification);

				attributeClassification->min.x = std::min(attributeClassification->min.x, double(point->extended_classification > 31 ? point->extended_classification : point->classification));
				attributeClassification->max.x = std::max(attributeClassification->max.x, double(point->extended_classification > 31 ? point->extended_classification : point->classification));
			};
